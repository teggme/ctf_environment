...
class BirdsEyeViewStaticObstacleLocationsEnvironment(gym.Env):
    def __init__(
        ...
        POSITIONAL_SET_POINT=np.array([0.0, 0.0], dtype=np.float32),
        train_hyrl=False,
        Mij_complete=None,
        j_train=0,
        ...
        USE_IMAGE=True,
        HORIZONTAL_MOVEMENT_AMPLITUDES=0.0,
        ...
        self.MINIMAL_VALUE_POSITION_X, self.MAXIMAL_VALUE_POSITION_X = -10.0, 10.0
        self.MINIMAL_VALUE_POSITION_Y, self.MAXIMAL_VALUE_POSITION_Y = -10.0, 10.0
        ...
        self.HORIZONTAL_POSITIONAL_SET_POINT, self.VERTICAL_POSITIONAL_SET_POINT = (
            POSITIONAL_SET_POINT[0],
            POSITIONAL_SET_POINT[1],
        )
        ...
        self.DYNAMIC_OBSTACLE_MOTION = (
            DYNAMIC_OBSTACLE_MOTION  # 'random', 'deterministic'
        )
        if self.DYNAMIC_OBSTACLE_MOTION == "deterministic":
            ...
            for obstacle_index in range(NUMBER_OF_DYNAMIC_OBSTACLES):
                ...
                self.COLLECTION_NUMBER_OF_HORIZONTAL_TERMS_OBSTACLE_DYNAMICS.append(
                    NUMBER_OF_HORIZONTAL_TERMS_OBSTACLE_DYNAMICS
                )
                self.COLLECTION_NUMBER_OF_VERTICAL_TERMS_OBSTACLE_DYNAMICS.append(
                    NUMBER_OF_VERTICAL_TERMS_OBSTACLE_DYNAMICS
                )
                self.COLLECTION_HORIZONTAL_MOVEMENT_AMPLITUDES.append(
                    HORIZONTAL_MOVEMENT_AMPLITUDES
                )
                self.COLLECTION_VERTICAL_MOVEMENT_AMPLITUDES.append(
                    VERTICAL_MOVEMENT_AMPLITUDES
                )
                self.COLLECTION_HORIZONTAL_MOVEMENT_PERIODS.append(
                    HORIZONTAL_MOVEMENT_PERIODS
                )
                self.COLLECTION_VERTICAL_MOVEMENT_PERIODS.append(
                    VERTICAL_MOVEMENT_PERIODS
                )
        ...
        if self.OBSTACLE_MODE == "dynamic" or self.OBSTACLE_MODE == "both":
            ...
            if type(DYNAMIC_OBSTACLE_RADII) == list:
                ...
                self.DYNAMIC_OBSTACLE_RADII = [DYNAMIC_OBSTACLE_RADII] * (
                    self.NUMBER_OF_DYNAMIC_OBSTACLES + 1
                )
        ...
        if self.VEHICLE_MODEL == "Dubin":
            ...
            self.low_state = np.array(
                [self.MINIMAL_VALUE_POSITION_X, self.MINIMAL_VALUE_POSITION_Y],
                dtype=np.float32,
            )
            self.high_state = np.array(
                [self.MAXIMAL_VALUE_POSITION_X, self.MAXIMAL_VALUE_POSITION_Y],
                dtype=np.float32,
            )
            ...
            self.observation_space = spaces.Dict(
                {
                    "image": spaces.Box(
                        ...
                        shape=(
                            self.NUMBER_OF_OBSERVATION_IMAGES,
                            self.RESOLUTION_BIRDSEYEVIEW_IMAGE,
                            self.RESOLUTION_BIRDSEYEVIEW_IMAGE,
                        ),
                        dtype=np.uint8,
                    ),
    ...
    def get_image_circle(
        self,
        horizontal_position_vehicle,
        vertical_position_vehicle,
        orientation_vector_vehicle,
    ):
        orientation_vehicle_angle = np.arctan2(
            orientation_vector_vehicle[1], orientation_vector_vehicle[0]
        )
        horizontal_values_image = np.linspace(
            horizontal_position_vehicle - self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE / 2,
            horizontal_position_vehicle + self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE / 2,
            self.RESOLUTION_BIRDSEYEVIEW_IMAGE,
        )
        vertical_values_image = np.linspace(
            vertical_position_vehicle - self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE / 2,
            vertical_position_vehicle + self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE / 2,
            self.RESOLUTION_BIRDSEYEVIEW_IMAGE,
        )
        horizontal_meshgrid_image, vertical_meshgrid_image = np.meshgrid(
            horizontal_values_image, vertical_values_image
        )
        image = np.zeros(
            (self.RESOLUTION_BIRDSEYEVIEW_IMAGE, self.RESOLUTION_BIRDSEYEVIEW_IMAGE)
        )

        if self.OBSTACLE_MODE == "static" or self.OBSTACLE_MODE == "both":
            for index_of_obstacle, obstacle_location in enumerate(
                self.STATIC_OBSTACLE_LOCATIONS
            ):
                horizontal_position_obstacle, vertical_position_obstacle = (
                    obstacle_location[0],
                    obstacle_location[1],
                )
                rotated_horizontal_values_image, rotated_vertical_values_image = (
                    self.rotate(
                        (horizontal_position_vehicle, vertical_position_vehicle),
                        (horizontal_position_obstacle, vertical_position_obstacle),
                        -orientation_vehicle_angle,
                    )
                )
                image += (
                    horizontal_meshgrid_image - rotated_horizontal_values_image
                ) ** 2 + (
                    vertical_meshgrid_image - rotated_vertical_values_image
                ) ** 2 < self.STATIC_OBSTACLE_RADII[
                    index_of_obstacle
                ] ** 2
        if self.OBSTACLE_MODE == "dynamic" or self.OBSTACLE_MODE == "both":
            for index_of_obstacle, obstacle_location in enumerate(
        ...
        return image.reshape(
            (1, self.RESOLUTION_BIRDSEYEVIEW_IMAGE, self.RESOLUTION_BIRDSEYEVIEW_IMAGE)
        )

    def get_distance_obs(
        self,
        obst,
        obstacle_radius,
        horizontal_position_vehicle,
        ...
        return max(
            np.sqrt(
                (horizontal_position_vehicle - obst[0]) ** 2
                + (vertical_position_vehicle - obst[1]) ** 2
            )
            - obstacle_radius,
            0.0,
        )
    ...
    def update_observation(
        ...
        horizontal_position_vehicle,
        vertical_position_vehicle,
        orientation_vector_vehicle=np.array([1, 0], dtype=np.float32),
    ):
        if self.VEHICLE_MODEL == "Dubin":
            desired_orientation = np.arctan2(
                self.VERTICAL_POSITIONAL_SET_POINT - vertical_position_vehicle,
                self.HORIZONTAL_POSITIONAL_SET_POINT - horizontal_position_vehicle,
            )
            vehicle_orientation_angle = np.arctan2(
                orientation_vector_vehicle[1], orientation_vector_vehicle[0]
            )
            ...
            distance_error = np.sqrt(
                (self.HORIZONTAL_POSITIONAL_SET_POINT - horizontal_position_vehicle)
                ** 2
                + (self.VERTICAL_POSITIONAL_SET_POINT - vertical_position_vehicle) ** 2
            )
            if self.USE_IMAGE == True:
                new_image = self.get_image_circle(
                    horizontal_position_vehicle,
                    vertical_position_vehicle,
                    orientation_vector_vehicle,
                )
                ...
                if self.NUMBER_OF_OBSERVATION_IMAGES > 1:
                    ...
                    for image_index in range(
                        self.NUMBER_OF_OBSERVATION_IMAGES - 1, 0, -1
                    ):
                        images[image_index] = images[image_index - 1]
                images[0] = new_image[0]
            observation_list = [distance_error, orientation_error]
            if self.OBSTACLE_MODE == "dynamic" or self.OBSTACLE_MODE == "both":
                for index_of_obstacle, dynamic_obstacle_location in enumerate(
                    self.DYNAMIC_OBSTACLE_LOCATIONS
                ):
                    for future_steps in range(self.FUTURE_STEPS_DYNAMIC_OBSTACLE + 1):
                        future_dynamic_obstacle_location = self.evolve_dynamic_obstacle(
                            dynamic_obstacle_location, index_of_obstacle, future_steps
                        )
                        distance_to_dynamic_obstacle = np.sqrt(
                            (
                                future_dynamic_obstacle_location[0]
                                - horizontal_position_vehicle
                            )
                            ** 2
                            + (
                                future_dynamic_obstacle_location[1]
                                - vertical_position_vehicle
                            )
                            ** 2
                        )
                        relative_orientation_to_dynamic_obstacle = np.arctan2(
                            future_dynamic_obstacle_location[1]
                            - vertical_position_vehicle,
                            future_dynamic_obstacle_location[0]
                            - horizontal_position_vehicle,
                        )
                        orientation_error_to_dynamic_obstacle = (
                            relative_orientation_to_dynamic_obstacle
                            - vehicle_orientation_angle
                        )
                        orientation_error_to_dynamic_obstacle = (
                            orientation_error_to_dynamic_obstacle + np.pi
                        ) % (2 * np.pi) - np.pi
        ...
        if self.USE_IMAGE == True:
            state = {"image": images, "vector": observation_vector}
        else:
            state = {"vector": observation_vector}
        return state

    def get_mindist_to_obs(
        self, horizontal_position_vehicle, vertical_position_vehicle
    ):
        ...
        if self.OBSTACLE_MODE == "static" or self.OBSTACLE_MODE == "both":
            for index_of_obstacle, obstacle_location in enumerate(
                ...
                d_obi = self.get_distance_obs(
                    obstacle_location,
                    self.STATIC_OBSTACLE_RADII[index_of_obstacle],
                    horizontal_position_vehicle,
                    vertical_position_vehicle,
                )
        ...
        if self.OBSTACLE_MODE == "dynamic" or self.OBSTACLE_MODE == "both":
            for index_of_obstacle, obstacle_location in enumerate(
                ...
                d_obi = self.get_distance_obs(
                    obstacle_location,
                    self.DYNAMIC_OBSTACLE_RADII[index_of_obstacle],
                    horizontal_position_vehicle,
                    vertical_position_vehicle,
                )
                if d_obi < distance_to_closest_obstacle:
    ...
    def check_bounds(
        self,
        new_horizontal_position_vehicle,
        new_vertical_position_vehicle,
        new_orientation_vector_vehicle=np.array([1, 0], dtype=np.float32),
    ):
        distance_to_closest_obstacle, _ = self.get_mindist_to_obs(
            new_horizontal_position_vehicle, new_vertical_position_vehicle
        )
        collision_with_obstacle = distance_to_closest_obstacle <= 0

        self.out_of_bounds = (
            collision_with_obstacle
            or abs(self.vertical_position_vehicle) >= self.MAXIMAL_VALUE_POSITION_Y
            or self.horizontal_position_vehicle > self.MAXIMAL_VALUE_POSITION_X
            or self.horizontal_position_vehicle < self.MINIMAL_VALUE_POSITION_X
        )

        if self.train_hyrl:
            if self.OBSTACLE_MODE == "static":
                conditions_for_being_close_to_obstacle = np.zeros(
                    len(self.STATIC_OBSTACLE_LOCATIONS)
                )
                for index_obstacle, obstacle_location in enumerate(
                    self.STATIC_OBSTACLE_LOCATIONS
                ):
                    horizontal_position_obstacle, vertical_position_obstacle = (
                        obstacle_location[0],
                    ...
                    conditions_for_being_close_to_obstacle[index_obstacle] = (
                        horizontal_position_obstacle - self.horizontal_position_vehicle
                    ) ** 2 + (
                        vertical_position_obstacle - self.vertical_position_vehicle
                    ) ** 2 < (
                        self.STATIC_OBSTACLE_RADII[index_obstacle]
                        + self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE / 2
                    ) ** 2
                in_Mij = np.zeros(len(self.STATIC_OBSTACLE_LOCATIONS))
                for idx_lambda in range(self.NUMBER_OF_STATIC_OBSTACLES):
                    in_Mij[idx_lambda] = self.Mij_complete.get_in_Mi(
                        (
                            self.horizontal_position_vehicle,
                            self.vertical_position_vehicle,
                        ),
                        self.j_train,
                        idx_lambda,
                    )
                self.out_of_bounds = self.out_of_bounds or (
                    np.sum(in_Mij) == 0
                    and np.sum(conditions_for_being_close_to_obstacle) >= 1
                )
            if self.OBSTACLE_MODE == "dynamic":
                conditions_for_being_close_to_obstacle = np.zeros(
                    len(self.DYNAMIC_OBSTACLE_LOCATIONS)
                )
                for index_obstacle, obstacle_location in enumerate(
                    self.DYNAMIC_OBSTACLE_LOCATIONS
                ):
                    ...
                    conditions_for_being_close_to_obstacle[index_obstacle] = (
                        ...
                        self.DYNAMIC_OBSTACLE_RADII[index_obstacle]
                        + self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE / 2
                    ) ** 2
                in_Mij = np.zeros(len(self.DYNAMIC_OBSTACLE_LOCATIONS))
                for idx, Mijs in enumerate(self.Mij_focused_obstacles):
                    in_Mij[idx] = Mijs.get_in_Mi(
                        (
                            self.horizontal_position_vehicle,
                            self.vertical_position_vehicle,
                        ),
                        self.j_train,
                        new_locations_obstacle=self.DYNAMIC_OBSTACLE_LOCATIONS[
                            self.focused_obstacle
                        ],
                        tau=self.time_elapsed,
                    )
                self.out_of_bounds = self.out_of_bounds or (
                    np.sum(in_Mij) == 0
                    and np.sum(conditions_for_being_close_to_obstacle) >= 1
        ...
        if self.out_of_bounds == False or self.clip_through_obstacles == True:
            velocity_magnitude = np.sqrt(
                (
                    (new_horizontal_position_vehicle - self.horizontal_position_vehicle)
                    / self.SAMPLING_TIME_SECONDS
                )
                ** 2
                + (
                    (new_vertical_position_vehicle - self.vertical_position_vehicle)
                    / self.SAMPLING_TIME_SECONDS
                )
                ** 2
            )
            ...
            if velocity_magnitude > 0.05:
                self.orientation_vector_vehicle = (
                    new_orientation_vector_vehicle
                    / np.linalg.norm(new_orientation_vector_vehicle)
                )
    ...
    def get_reward(self, state):
        ...
        if self.VEHICLE_MODEL == "pointmass":
            ...
            distance_to_positional_set_point = np.sqrt(
                horizontal_position_error**2 + vertical_position_error**2
            )
        else:
            distance_to_positional_set_point = observation_vector[0]

        distance_to_closest_obstacle, index_of_obstacle = self.get_mindist_to_obs(
            self.horizontal_position_vehicle, self.vertical_position_vehicle
        )
        if self.train_hyrl:
            if self.OBSTACLE_MODE == "static":
                boundary_hyrl_kwargs = dict(
                    point=(
                        self.horizontal_position_vehicle,
                        self.vertical_position_vehicle,
                    ),
            ...
            if self.OBSTACLE_MODE == "dynamic":
                ...
                )  # TODO extend to static + dynamic

            if self.USE_TIMER:
                boundary_hyrl_kwargs["tau"] = self.time_elapsed

            distance_to_boundary_hyrl_sets = (
                self.Mij_complete.distance_to_boundary_hyrl(
                    (self.horizontal_position_vehicle, self.vertical_position_vehicle),
                    self.j_train,
                    self.focused_obstacle,
                )
            )
            distance_to_closest_obstacle = min(
                distance_to_closest_obstacle, distance_to_boundary_hyrl_sets
            )
        ...
        if distance_to_closest_obstacle < self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE / 2:
            ...
            barrier_function = (
                ...
                - 0.5 * self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE
            ) ** 2 * np.log(
                0.5
                * self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE
                / distance_to_closest_obstacle
            )
        else:
            barrier_function = 0
        ...
        reward_time_step = (
            -distance_to_positional_set_point * self.SAMPLING_TIME_SECONDS
            - BARRIER_PENALTY * barrier_function * self.SAMPLING_TIME_SECONDS
        )
        if self.VEHICLE_MODEL == "Dubin":
    ...
    def evolve_dynamic_obstacle(
        self, dynamic_obstacle_location, dynamic_obstacle_index, future_steps
    ):
        time_point = self.time_elapsed + future_steps * self.SAMPLING_TIME_SECONDS
        INITIAL_DYNAMIC_OBSTACLE_LOCATION = self.INITIAL_DYNAMIC_OBSTACLE_LOCATIONS[
        ...
        for term in range(
            ...
            NEW_HORIZONTAL_POSITION_DYNAMIC_OBSTACLE += (
                self.COLLECTION_HORIZONTAL_MOVEMENT_AMPLITUDES[dynamic_obstacle_index][
                    term
                ]
                * np.sin(
                    time_point
        ...
        for term in range(
            ...
            NEW_VERTICAL_POSITION_DYNAMIC_OBSTACLE += (
                self.COLLECTION_VERTICAL_MOVEMENT_AMPLITUDES[dynamic_obstacle_index][
                    term
                ]
                * np.sin(
                    time_point
                    * 2
                    * np.pi
                    / self.COLLECTION_VERTICAL_MOVEMENT_PERIODS[dynamic_obstacle_index][
                        term
                    ]
                )
            )
        NEW_DYNAMIC_OBSTACLE_LOCATION = (
            NEW_HORIZONTAL_POSITION_DYNAMIC_OBSTACLE,
            NEW_VERTICAL_POSITION_DYNAMIC_OBSTACLE,
        )
    ...
    def dynamics_vehicle(self, time, vehicle_state, action):
        if self.VEHICLE_MODEL == "Dubin":
            ...
            control_input_forward_velocity = (
                action[0] * self.MAGNITUDE_CONTROL_INPUT_FORWARD_VELOCITY
            )
            control_input_orientation = (
                action[1] * self.MAGNITUDE_CONTROL_INPUT_ORIENTATION
            )
            timederivative_orientation_vector_vehicle = (
                control_input_orientation
                * np.array(
                    [orientation_vector_vehicle[1], -orientation_vector_vehicle[0]]
                )
            )
            timederivative_horizontal_position_vehicle = (
                control_input_forward_velocity * orientation_vector_vehicle[0]
            )
            timederivative_vertical_position_vehicle = (
                control_input_forward_velocity * orientation_vector_vehicle[1]
            )
            timederivative_vehicle_state = [
                timederivative_horizontal_position_vehicle,
                timederivative_vertical_position_vehicle,
            ] + list(timederivative_orientation_vector_vehicle)
        elif self.VEHICLE_MODEL == "pointmass":
            control_input_horizontal_velocity = (
                action[0] * self.MAGNITUDE_CONTROL_INPUT_FORWARD_VELOCITY
            )
            control_input_vertical_velocity = (
                action[1] * self.MAGNITUDE_CONTROL_INPUT_FORWARD_VELOCITY
            )
            timederivative_horizontal_position_vehicle = (
                control_input_horizontal_velocity
            )
    ...
    def step(self, action):
        ...
        if self.OBSTACLE_MODE == "dynamic" or self.OBSTACLE_MODE == "both":
            ...
            INITIAL_DYNAMIC_OBSTACLE_LOCATIONS = (
                self.INITIAL_DYNAMIC_OBSTACLE_LOCATIONS.copy()
            )
            for index_of_obstacle, _ in enumerate(self.DYNAMIC_OBSTACLE_LOCATIONS):
                NEW_DYNAMIC_OBSTACLE_LOCATION = self.evolve_dynamic_obstacle(
                    _, index_of_obstacle, 0
                )
        ...
        vehicle_state = [
            self.horizontal_position_vehicle,
            self.vertical_position_vehicle,
        ] + list(self.orientation_vector_vehicle)
        timederivative_vehicle_state = self.dynamics_vehicle(0, vehicle_state, action)
        if self.VEHICLE_MODEL == "Dubin":
            new_orientation_vector_vehicle = (
                self.orientation_vector_vehicle
                + np.array(timederivative_vehicle_state[2:])
                * self.SAMPLING_TIME_SECONDS
            )
            new_horizontal_position_vehicle = (
                self.horizontal_position_vehicle
                + timederivative_vehicle_state[0] * self.SAMPLING_TIME_SECONDS
            )
            new_vertical_position_vehicle = (
                self.vertical_position_vehicle
                + timederivative_vehicle_state[1] * self.SAMPLING_TIME_SECONDS
            )
    ...
    def get_initialize_state_randomly(self):
        if self.initialize_initial_state_method == "random":
            ...
            while stop_loop == False:
                self.horizontal_position_vehicle = np.random.uniform(
                    self.MINIMAL_VALUE_POSITION_X + 1, self.MAXIMAL_VALUE_POSITION_X - 1
                )
                self.vertical_position_vehicle = np.random.uniform(
                    self.MINIMAL_VALUE_POSITION_Y + 1, self.MAXIMAL_VALUE_POSITION_Y - 1
                )
                if self.OBSTACLE_MODE == "static":
                    ...
                    OBSTACLE_LOCATIONS_INITIALIZE = (
                        self.STATIC_OBSTACLE_LOCATIONS + self.DYNAMIC_OBSTACLE_LOCATIONS
                    )
                    OBSTACLE_RADII_INITIALIZE = (
                        self.STATIC_OBSTACLE_RADII + self.DYNAMIC_OBSTACLE_RADII
                    )
                else:
                    print("no valid obstacle mode passed")
                conditions_for_being_close_to_obstacle = np.zeros(
                    len(OBSTACLE_LOCATIONS_INITIALIZE)
                )
                conditions_for_being_far_from_obstacle = np.zeros(
                    len(OBSTACLE_LOCATIONS_INITIALIZE)
                )
                conditions_for_being_far_from_positional_set_point = np.zeros(
                    len(OBSTACLE_LOCATIONS_INITIALIZE)
                )
                conditions_for_being_close_to_positional_set_point = np.zeros(
                    len(OBSTACLE_LOCATIONS_INITIALIZE)
                )
                for index_obstacle, obstacle_location in enumerate(
                    OBSTACLE_LOCATIONS_INITIALIZE
                    ...
                    conditions_for_being_close_to_obstacle[index_obstacle] = (
                        ...
                        OBSTACLE_RADII_INITIALIZE[index_obstacle] * 1.1
                    ) ** 2  # 2 -> 1.5 -> 1.1
                    conditions_for_being_far_from_positional_set_point[
                        index_obstacle
                    ] = (
                        self.horizontal_position_vehicle
                        - self.HORIZONTAL_POSITIONAL_SET_POINT
                    ) ** 2 + (
                        self.vertical_position_vehicle
                        - self.VERTICAL_POSITIONAL_SET_POINT
                    ) ** 2 > (
                        OBSTACLE_RADII_INITIALIZE[index_obstacle] * 2
                    ) ** 2  # 4 --> 2
                    conditions_for_being_far_from_obstacle[index_obstacle] = (
                        horizontal_position_obstacle - self.horizontal_position_vehicle
                    ) ** 2 + (
                        vertical_position_obstacle - self.vertical_position_vehicle
                    ) ** 2 < (
                        2 * self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE
                    ) ** 2  # (OBSTACLE_RADII_INITIALIZE[index_obstacle]*18)**2 # 5 -> 2 -> 18 -> 5
                    conditions_for_being_close_to_positional_set_point[
                        index_obstacle
                    ] = (
                        self.horizontal_position_vehicle
                        - self.HORIZONTAL_POSITIONAL_SET_POINT
                ...
                if self.train_hyrl:
                    if self.OBSTACLE_MODE == "static":
                        ...
                        for idx_lambda in range(self.NUMBER_OF_STATIC_OBSTACLES):
                            if self.init_in:
                                ...
                                in_Mij[idx_lambda] = self.Mij_complete.near_boundary(
                                    (
                                        self.horizontal_position_vehicle,
                                        self.vertical_position_vehicle,
                                    ),
                                    self.j_train,
                                    index_lambda=self.focused_obstacle,
                                    boundary_margin=self.margin,
                                )
                        if (
                            np.prod(conditions_for_being_close_to_obstacle) > 0
                            and np.prod(
                                conditions_for_being_far_from_positional_set_point
                            )
                            > 0
                            and np.sum(conditions_for_being_far_from_obstacle) >= 1
                            and np.sum(in_Mij) >= 1
                            and np.prod(
                                conditions_for_being_close_to_positional_set_point
                            )
                    ...
                    if self.OBSTACLE_MODE == "dynamic":
                        ...
                        if self.OBSTACLE_MODE == "both":
                            Exception(
                                "Hyrl for dynamic + static obstacles not embedded yet"
                            )
                else:
                    if (
                        np.prod(conditions_for_being_close_to_obstacle) > 0
                        and np.prod(conditions_for_being_far_from_positional_set_point)
                        > 0
                        and np.prod(conditions_for_being_close_to_positional_set_point)
                        > 0
                    ):
                        stop_loop = True
    ...
    def get_static_obstacle_locations(self):
        ...
        angle_spread_obs1 = np.linspace(
            start_angle,
            2 * np.pi + start_angle,
            min(4, self.NUMBER_OF_STATIC_OBSTACLES),
            endpoint=False,
        )
        if self.NUMBER_OF_STATIC_OBSTACLES > 4:
            start_angle2 = np.random.uniform(0, 2 * np.pi)
            angle_spread_obs2 = np.linspace(
                start_angle2,
                2 * np.pi + start_angle2,
                min(12, self.NUMBER_OF_STATIC_OBSTACLES - 4),
                endpoint=False,
            )
        self.STATIC_OBSTACLE_LOCATIONS = []
        for obstacle_number in range(self.NUMBER_OF_STATIC_OBSTACLES):
            if obstacle_number < 4:
                obstacle_initialization_angle = np.random.uniform(
                    -0.1 * np.pi, 0.1 * np.pi
                )
                distance_obs = np.random.uniform(
                    self.STATIC_OBSTACLE_RADII[obstacle_number] * 3.5,
                    self.STATIC_OBSTACLE_RADII[obstacle_number] * 4.5,
                )
                horizontal_position_obstacle = distance_obs * np.cos(
                    obstacle_initialization_angle + angle_spread_obs1[obstacle_number]
                )
                vertical_position_obstacle = distance_obs * np.sin(
                    obstacle_initialization_angle + angle_spread_obs1[obstacle_number]
                )
            else:
                obstacle_number -= 4
                ...
                distance_obs = np.random.uniform(
                    self.STATIC_OBSTACLE_RADII[obstacle_number] * 8,
                    self.STATIC_OBSTACLE_RADII[obstacle_number] * 10,
                )
                horizontal_position_obstacle = distance_obs * np.cos(
                    obstacle_initialization_angle + angle_spread_obs2[obstacle_number]
                )
                vertical_position_obstacle = distance_obs * np.sin(
                    obstacle_initialization_angle + angle_spread_obs2[obstacle_number]
                )
            self.STATIC_OBSTACLE_LOCATIONS.append(
                (horizontal_position_obstacle, vertical_position_obstacle)
            )

    def get_dynamic_obstacle_locations(self):
        print(
            "Functionality not added yet, define the locations of the moving obstacles"
        )
        print("using locations (-2, 0), (2, 0)")
        standard_locations = [(-2, 0), (2, 0)]
        self.DYNAMIC_OBSTACLE_LOCATIONS = standard_locations[
            : self.NUMBER_OF_DYNAMIC_OBSTACLES
        ]
    ...
    def reset(self, seed=None):
        ...
        if self.train:
            self.time_elapsed = np.random.uniform(
                0, self.TIME_STEPS * self.SAMPLING_TIME_SECONDS
            )
        ...
        if self.OBSTACLE_MODE == "dynamic" or self.OBSTACLE_MODE == "both":
            if self.INITIAL_DYNAMIC_OBSTACLE_LOCATIONS is None:
                self.INITIAL_DYNAMIC_OBSTACLE_LOCATIONS = list(
                    self.DYNAMIC_OBSTACLE_LOCATIONS
                )
        ...
        if self.train_hyrl:
            if self.INITIAL_FOCUSED_OBSTACLE is None:
                if self.OBSTACLE_MODE == "static":
                    self.focused_obstacle = np.random.randint(
                        self.NUMBER_OF_STATIC_OBSTACLES
                    )
                if self.OBSTACLE_MODE == "dynamic":
                    self.focused_obstacle = np.random.randint(
                        self.NUMBER_OF_DYNAMIC_OBSTACLES
        ...
        self.horizontal_position_vehicle = min(
            max(self.INITIAL_STATE[0], self.MINIMAL_VALUE_POSITION_X),
            self.MAXIMAL_VALUE_POSITION_X,
        )
        self.vertical_position_vehicle = min(
            max(self.INITIAL_STATE[1], self.MINIMAL_VALUE_POSITION_Y),
            self.MAXIMAL_VALUE_POSITION_Y,
        )
        if self.VEHICLE_MODEL == "Dubin":
            self.orientation_vector_vehicle = np.array(
                [self.INITIAL_STATE[2], self.INITIAL_STATE[3]]
            )
        if self.initialize_state_randomly == True:
            ...
            if self.VEHICLE_MODEL == "Dubin":
                orientation_vehicle_angle = np.arctan2(
                    self.VERTICAL_POSITIONAL_SET_POINT - self.vertical_position_vehicle,
                    self.HORIZONTAL_POSITIONAL_SET_POINT
                    - self.horizontal_position_vehicle,
                )
                orientation_vehicle_angle += np.random.uniform(
                    low=-np.pi / 2, high=np.pi / 2
                )  # * 4 # TODO
                (
                    self.orientation_vector_vehicle[0],
                    self.orientation_vector_vehicle[1],
                ) = np.cos(orientation_vehicle_angle), np.sin(orientation_vehicle_angle)

        if self.VEHICLE_MODEL == "Dubin":
            if self.USE_IMAGE == True:
                initial_images = np.zeros(
                    (
                        self.NUMBER_OF_OBSERVATION_IMAGES,
                        self.RESOLUTION_BIRDSEYEVIEW_IMAGE,
                        self.RESOLUTION_BIRDSEYEVIEW_IMAGE,
                ...
                initial_image = self.get_image_circle(
                    self.horizontal_position_vehicle,
                    self.vertical_position_vehicle,
                    self.orientation_vector_vehicle,
                )
