    ...
    for iii in range(iterations):
                ...
                for n_rod in range(n_rodpoints):
                    ...
                    for start in start_points:
                        ...
                        if custom_state_init is not None:
                            start = custom_state_init(
                                start, model, env_kwargs["POSITIONAL_SET_POINT"]
                            )
                        ...
                        length_shortest_trajectory = min(
                            len(trajectory), length_shortest_trajectory
                        )
                    ...
                    if ignore_point == False:
                        ...
                        for step_traj in range(length_shortest_trajectory):
                            differences.append(
                                max(
                                    np.linalg.norm(
                                        trajectories[0][step_traj]
                                        - trajectories[1][step_traj]
                                    )
                                    - initial_difference,
                                    0,
                                )
                                * env.SAMPLING_TIME_SECONDS
                            )
        ...
        if iii < iterations - 1:
            if len(valid_points) > n_clusters:
                ...
                kmeans = KMeans(
                    n_clusters=min(n_clusters, len(valid_points)), random_state=0
                ).fit(cluster_array)
...
class Def_Mob_general:
    ...
    def __init__(
        ...
        WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE=2,
    ):
    ...
    def set_voronoi_partition(
        self, STATIC_OBSTACLE_LOCATIONS, STATIC_OBSTACLE_RADII=0.5
    ):
    ...
    def check_voronoi(self, point, index_lambda, res=50):
        ...
        if nearest_point_index == index_lambda:
            return True
        distance_to_nearest_obstacle = np.linalg.norm(
            self.vor.points[nearest_point_index] - point
        )
        if (
            distance_to_nearest_obstacle
            - self.vor.STATIC_OBSTACLE_RADII[nearest_point_index]
            <= np.sqrt(2) * self.WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE / 2
        ):
            return False
        ...
        for alpha in alphas:
            near_point = np.array(
                [x, y]
            ) + self.vor_margin_factor * distance_to_nearest_obstacle * np.array(
                [np.cos(alpha), np.sin(alpha)]
            )
            point_index = np.argmin(np.sum((self.vor.points - near_point) ** 2, axis=1))
    ...
    def transform_coordinates(self, position_to_transform, targeted_obstacle):
        polar_position_to_transform = self.cart2pol(
            position_to_transform - self.POSITIONAL_SET_POINT
        )
        targeted_obstacle_setpoint_adjusted = (
            targeted_obstacle - self.POSITIONAL_SET_POINT
        )
        ...
        transformed_polar = (
            polar_position_to_transform[0] / scale_to_transform,
            polar_position_to_transform[1] - angle_to_transform,
        )
    ...
    def near_boundary(
        self, point, index_q, index_lambda, boundary_margin=0.3, RESOLUTION_SEARCH=10
    ):
        alphas = np.linspace(0, 2 * np.pi, num=RESOLUTION_SEARCH, endpoint=False)
        in_Mij = []
        for alpha in alphas:
            near_point = (
                point + np.array([np.cos(alpha), np.sin(alpha)]) * boundary_margin
            )
    ...
    def distance_to_boundary_hyrl(
        ...
        if self.get_in_Mi(point, index_q, index_lambda):
            return (
                WIDTH_AND_LENGTH_BIRDSEYEVIEW_IMAGE / 2 + 1
            )  # setting distance larger than observaiton window
        ...
        for alpha in alphas:
            near_point = (
                point
                + np.array([np.cos(alpha), np.sin(alpha)]) * min_distance_to_boundary
            )
        ...
        for _ in range(RESOLUTION_SEARCH):
            ...
            for alpha in alphas:
                near_point = (
                    point + np.array([np.cos(alpha), np.sin(alpha)]) * mid_range
                )
                if self.get_in_Mi(near_point, index_q, index_lambda):
                    near_boundary = True
                    max_range = mid_range
